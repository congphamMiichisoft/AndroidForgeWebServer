...
/*
   ...
   */
proto.compareTwoViewables = function (lmvDocumentA, bubbleNodeA, lmvDocumentB, bubbleNodeB, cbAfterFirstModel, cbAfterSecondModel) {
  return this._compareTwoThings('_loadViewable', lmvDocumentA, bubbleNodeA, lmvDocumentB, bubbleNodeB, cbAfterFirstModel, cbAfterSecondModel);
};
...
proto._loadViewable = function (doc, bubbleNode, isFirst, cb) {var _this = this;
  return new Promise(function (resolve, reject) {
    _this.viewer.loadDocumentNode(doc, bubbleNode, { keepCurrentModels: true, loadAsHidden: true }).then(function (model) {
      if (cb(model)) {
        resolve(true);
      } else {
...
proto._loadDoc = function (doc, itemId, isFirst, cb) {

  var bubbleNode = this._getDocGeometry(doc, itemId);

  return this._loadViewable(doc, bubbleNode, isFirst, cb);
};

/*
    * Loads a 2D model
    */
proto._loadModel = function (urn, itemId, isFirst, cb) {
  var self = this;

  return new Promise(function (resolve, reject) {

    var onLoad = function onLoad(doc) {
      self._loadDoc(doc, itemId, isFirst, cb).then(function () {
        resolve(true);
      }).catch(function (err) {
        reject(err);
      });
